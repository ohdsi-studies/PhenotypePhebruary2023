...
version <- "v0.1.0"
name <- "phenotypePhebruary2023 - an OHDSI network study"
packageName <- "phenotypePhebruary2023"
...
description <-
  "Cohort diagnostics on phenotypePhebruary2023 cohorts."


# Set up# from atlas-phenotype.ohdsi.org
...
studyCohorts <- dplyr::bind_rows(
  ...
  webApiCohorts |>
    dplyr::filter(
      stringr::str_detect(string = tolower(name),
                          pattern = "neurofibromatosis")
    )
)
